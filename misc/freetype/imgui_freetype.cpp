...
static bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked, void* loader_data_for_baked_src)
{
    ...
    const bool prefer_bitmap_strike = (bd_font_data->UserFlags & ImGuiFreeTypeLoaderFlags_Bitmap) != 0 && face->num_fixed_sizes > 0;
    if (prefer_bitmap_strike)
    {
        const float requested_ppem = size * rasterizer_density;
        int best_size_index = 0;
        float best_size_delta = FLT_MAX;
        for (int size_index = 0; size_index < face->num_fixed_sizes; ++size_index)
        {
            const float strike_ppem = (float)face->available_sizes[size_index].y_ppem / 64.0f;
            const float delta = ImFabs(strike_ppem - requested_ppem);
            if (delta < best_size_delta)
            {
                best_size_delta = delta;
                best_size_index = size_index;
            }
        }
        FT_Select_Size(face, best_size_index);
    }
    else
    {
